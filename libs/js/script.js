$(function() {
	...
	$('#location, #reference, #designation, #quantity, #comment, #block, #history-ref, #history-des, #date-one, #date-first, #date-last, #new-plus-qty, #new-minus-qty, #new-transfer-loc, #update-comment').on('focus', function() {
		$(this).select();
	});
	...
	$('#add-form').submit(function(e) {
		...
		$.ajax({
            ...
            success: function(json) {
                ...
                if ( json.isSuccess ) 
                {
                    ...
                    $.ajax({
			        	...
			        	success: function(json) {
							...
							if ( !json.isLocationEmpty && !json.isEntryOnLocation )
			        		{
								$('#modal-confirm').on('show.bs.modal', function () {
									var modal = $(this);
									modal.find('.modal-title').text('Emplacement ' + $('#location').val() + ' déjà occupé');
									modal.find('.modal-body p:eq(0)').text('Voulez-vous quand même gerber le produit');
									modal.find('.modal-body p:eq(1)').text($('#designation').val());
									modal.find('.modal-body p:eq(2)').text('à cet emplacement ?');
								});
							...
							else if ( !json.isLocationEmpty && json.isEntryOnLocation )
							{
								$('#modal-confirm').on('show.bs.modal', function () {
									var modal = $(this);
									modal.find('.modal-title').text('Produit déjà présent sur l\'emplacement ' + $('#location').val());
									modal.find('.modal-body p:eq(0)').text('Voulez-vous quand même gerber le produit');
									modal.find('.modal-body p:eq(1)').text($('#designation').val());
									modal.find('.modal-body p:eq(2)').text('à ce même emplacement ?');
								});
                ...
                {
                    $('#add-loc, #add-qty, #add-ref, #add-des').addClass('has-error');
	...
	$('#transfer-button').on('click', function(e) {
		...
		$.ajax({
		 	type: 'POST',
		 	url: '../../ajax/checkTransfer.php',
		 	data: postdata,
		 	dataType: 'json',
		 	success: function(json) {
				if ( json.isSuccess && json.isLocationEmpty )
				{
					$('#new-input-transfer-loc').removeClass('has-error');
					$('#help-transfer-loc').html();
					$.ajax({
						type: 'POST',
				...
				else if ( json.isSuccess && !json.isLocationEmpty && !json.isEntryOnLocation )
				{
					$('#new-input-transfer-loc').removeClass('has-error');
					$('#help-transfer-loc').html();
					...
					$('#modal-confirm').on('show.bs.modal', function () {
						var modal = $(this);
						modal.find('.modal-title').text('Emplacement ' + $('#new-transfer-loc').val() + ' déjà occupé');
						modal.find('.modal-body p:eq(0)').text('Voulez-vous quand même gerber le produit');
						modal.find('.modal-body p:eq(1)').text($('#transfer-des').val());
						modal.find('.modal-body p:eq(2)').text('à cet emplacement ?');
					});
				...
				else if ( json.isSuccess && !json.isLocationEmpty && json.isEntryOnLocation )
				{
					$('#modal-confirm').on('show.bs.modal', function () {
						var modal = $(this);
						modal.find('.modal-title').text('Produit déjà présent sur l\'emplacement ' + $('#new-transfer-loc').val());
						modal.find('.modal-body p:eq(0)').text('Voulez-vous quand même gerber le produit');
						modal.find('.modal-body p:eq(1)').text($('#transfer-des').val());
						modal.find('.modal-body p:eq(2)').text('à ce même emplacement ?');
					});
	...
	// Fonction qui échappe les caractères spéciaux pour l'url
	...
	if ( $(location).attr('pathname') == '/productEntry.php' )
	{
		...
		$("#reference").autocomplete({
	        ...
	        source: function(request, response) {
						...
						$.ajax({
							type: 'GET',
							url: 'http://sccoreapi/v1/product/?filter={"is_stock_managed":' + stock_managed + ',"id__startswith":"' + encodeURI(request.term) + '"}&rows=50',
							data: {
								term: request.term
							},
							dataType: 'json',
	    ...
	    $("#designation").autocomplete({
	      	...
	      	source: function(request, response) {
						...
						$.ajax({
							type: 'GET',
							url: 'http://sccoreapi/v1/product/?filter={"is_stock_managed":' + stock_managed + ',"name__contains":"' + encodeURI(escapeStr(request.term)) + '"}&rows=50',
							data: {
								term: request.term
							},
							dataType: 'json',
	...
	if ( $(location).attr('pathname') == '/history.php' )
	{
		...
		$("#history-ref").autocomplete({
	        ...
	        source: function(request, response) {
						...
						$.ajax({
							type: 'GET',
							url: 'http://sccoreapi/v1/product/?filter={"id__startswith":"' + encodeURI(request.term) + '"}&rows=50',
							data: {
								term: request.term
							},
							dataType: 'json',
	    ...
	    $("#history-des").autocomplete({
	      	...
	      	source: function(request, response) {
						...
						$.ajax({
							type: 'GET',
							url: 'http://sccoreapi/v1/product/?filter={"name__contains":"' + encodeURI(request.term) + '"}&rows=50',
							data: {
								term: request.term
							},
							dataType: 'json',
